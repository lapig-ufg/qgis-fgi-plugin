...
class Test(unittest.TestCase):
    ...
    def get_test_parser(cls):
        ...
        parser = SatUtilsParser.newbie(description='satsearch testing')
        return parser
    ...
    def test_cli(self):
        """ Run CLI program """
        with patch.object(sys, 'argv', 'satsearch search --datetime 2017-01-01 --found -q platform=sentinel-2b'.split(' ')):
            cli()

    def test_cli_intersects(self):
        cmd = 'satsearch search --intersects %s -q platform=sentinel-2b --found' % os.path.join(testpath, 'aoi1.geojson')
